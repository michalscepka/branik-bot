    ...
    public static Dictionary<decimal, string> ExtractPrices(this string message)
    {
        ...
        foreach (Match match in matches)
        {
            ...
            if (match.Groups[nameof(PriceGroup.Ones)].Success)
                result.TryAdd(decimal.Parse(match.Groups[nameof(PriceGroup.Ones)].Value), originalValue);

            if (match.Groups[nameof(PriceGroup.Thousands)].Success)
                result.TryAdd(decimal.Parse(match.Groups[nameof(PriceGroup.Thousands)].Value) * 1_000, originalValue);

            if (match.Groups[nameof(PriceGroup.Millions)].Success)
                result.TryAdd(decimal.Parse(match.Groups[nameof(PriceGroup.Millions)].Value) * 1_000_000, originalValue);
        }
